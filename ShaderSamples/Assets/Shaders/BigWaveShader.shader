...
Shader "Custom/URPWaterLit_BigWaveNoise"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            //     // X ve Z yönleri için yüksekliği hesapla
            //     float yX = sin(pos.x * _BigWaveFrequency + _Time.y * _BigWaveSpeed * (1+nSpeed*_NoiseAmplitudeSpeed)) * (_BigWaveAmplitude * (1+nX*_NoiseAmplitudeX));
            //     float yZ = sin(pos.z * _BigWaveFrequency + _Time.y * _BigWaveSpeed * (1+nSpeed*_NoiseAmplitudeSpeed)) * (_BigWaveAmplitude * (1+nZ*_NoiseAmplitudeZ));

            //     return yX + yZ;
            // }
            float GetBigWave(float3 pos)
            {
                // X, Z ve hız için ayrı noise
                float nX = (noise(pos.xz * _NoiseScaleX + _Time.y*0.1) - 0.5) * 2.0;
                float nZ = (noise(pos.xz * _NoiseScaleZ + _Time.y*0.2) - 0.5) * 2.0;
                float nSpeed = (noise(pos.xz * _NoiseScaleSpeed + _Time.y*0.3) - 0.5) * 2.0;

                // Dalga yüksekliği ve hızını çarpma ile modüle et
